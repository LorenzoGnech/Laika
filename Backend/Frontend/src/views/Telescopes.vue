<template>
  <div id="app">
    ...
    <div class="container" ref="gridcontainer">
       <HCardGrid :cards=cTelescopes :cardsHeight="getCardsHeight" :cardsWidth="getCardsWidth" type="telescope"/>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    keepElementWithTag(t){
      let helper = this.cTelescopes;
      this.cTelescopes = [];
      for(let i = 0; i<helper.length; i++){
        if(helper[i].tags.includes(t)){
          this.cTelescopes.push(helper[i]);
        }
      }
    },
    searchQuery(q){
